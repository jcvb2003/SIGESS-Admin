-- portarias: segmentação funcional de sócios por portaria de defeso
-- Equivalente a localidades: filtro operacional, nunca fronteira de RLS/isolamento
-- unit_id NULLABLE: preenchido quando há polos, NULL em topologias sem polo
...
CREATE TABLE IF NOT EXISTS public.portarias (
  ...
  unit_id          uuid,                          -- NULL = topologia sem polo
...
-- Unicidade: dois índices parciais para cobrir ambos os cenários
...
COMMENT ON TABLE public.portarias IS
  'Segmentacao operacional de socios por portaria de defeso. Filtro funcional — nunca usada como fronteira de RLS.';
COMMENT ON COLUMN public.portarias.unit_id IS
  'NULL em topologias sem polo (isolated/shared single). Preenchido quando ha polos reais.';
