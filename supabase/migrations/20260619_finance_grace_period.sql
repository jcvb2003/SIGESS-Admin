...
ALTER TABLE public.financeiro_config_socio
...
REVOKE ALL ON FUNCTION public.get_socio_financial_status(text, text, boolean, boolean, date) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_socio_financial_status(text, text, boolean, boolean, date) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_socio_financial_status(text, text, boolean, boolean, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_socio_financial_status(text, text, boolean, boolean, date) TO service_role;
...
SELECT
  ...
  anuidades_pagas,
  ultimo_pagamento,
  public.get_socio_financial_status(
    cpf,
    regime,
    isento,
    liberado_presidente,
    data_efetiva_inicio_cobranca
  ) AS situacao_geral,
  meses_pagos_atual,
  data_de_admissao
FROM base;
